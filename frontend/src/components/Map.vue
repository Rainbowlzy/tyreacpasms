...
<script>
    ...
    export default {
        ...
        mounted: function () {
            var vm = this;
            this.$http.get('http://122.193.9.83/XiangXi/DefaultHandler.ashx?method=GetMapCategoryEvaluator').then(function (categories) {
...
//                    new AMap.TileLayer()//高德默认标准图层
            ...
            // 据说要不到点位，所以随便画好了。
            var address = "香溪社区外框图";
            this.$http.get("http://122.193.9.83/XiangXi/DefaultHandler.ashx?method=getpoilist", {
                params: {
        ...
        methods: {
            ...
            toggle_sub: function (sub) {
                ...
                $('.map_right_nav').hide();
                $('.map_right_com').hide();
                this.$http.get('http://122.193.9.83/XiangXi/DefaultHandler.ashx?method='+option.method, {
                    params:{
