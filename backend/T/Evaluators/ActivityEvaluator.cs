...
{
    public partial class DeleteActivityEvaluator : Evaluator
    {
        protected override object Evaluate(CommonRequest request)
        {
            var data = JsonConvert.DeserializeObject<Activity>(request.data);
            if (data == null)
            ...
            {
                var one = ctx.Activity.Find(data.id);
				if(one==null){
					return new CommonOutputT<string>
					{
						success = false,
    ...
    public partial class SaveActivityEvaluator : Evaluator
    {
        protected override object Evaluate(CommonRequest request)
        {
            if (Sessions.Count==0)
            ...
            {
				var s = request.data;
				var entity = JsonConvert.DeserializeObject<Activity>(s);
				// 行级排他锁开始
                var transactionId = Guid.NewGuid().ToString();
                var isnew = entity.id == null;
                if (!isnew)
                {
                    var one = ctx.Activity.FirstOrDefault(p=>p.id==entity.id);
                ...
                entity.UpdateOn = now;
                entity.IsDeleted = 0;
				ctx.Activity.AddOrUpdate(entity);
                ctx.SaveChanges();
				// 行级排他锁结束
                return new
                {
    ...
    public partial class GetActivityEmptyEvaluator:Evaluator
    {
        protected override object Evaluate(CommonRequest request)
        {
            return new Activity();
    ...
    public partial class GetActivityListEvaluator : Evaluator
    {
        protected override object Evaluate(CommonRequest request)
        {
            using (var ctx = new SPCall())
            {
                ...
                if (sppara != null)
                {
                    rows=ctx.SP_GET_ALL_LIST<Activity>(sppara);
                    return new
                    {
                        success = true,
                        rows = rows,
                ...
                rows = ctx.SP_GET_ALL_LIST<Activity>("", "", "", "", "", 0, 10, out total);
                return new
                {
                    success = true,
                    rows = rows,
