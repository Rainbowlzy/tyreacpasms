...
{
    public partial class DeleteTrafficEvaluator : Evaluator
    {
        protected override object Evaluate(CommonRequest request)
        {
            var data = JsonConvert.DeserializeObject<Traffic>(request.data);
            if (data == null)
            ...
            {
                var one = ctx.Traffic.Find(data.id);
				if(one==null){
					return new CommonOutputT<string>
					{
						success = false,
    ...
    public partial class SaveTrafficEvaluator : Evaluator
    {
        protected override object Evaluate(CommonRequest request)
        {
            if (Sessions.Count==0)
            ...
            {
				var s = request.data;
				var entity = JsonConvert.DeserializeObject<Traffic>(s);
				// 行级排他锁开始
                var transactionId = Guid.NewGuid().ToString();
                var isnew = entity.id == null;
                if (!isnew)
                {
                    var one = ctx.Traffic.FirstOrDefault(p=>p.id==entity.id);
                ...
                entity.UpdateOn = now;
                entity.IsDeleted = 0;
				ctx.Traffic.AddOrUpdate(entity);
                ctx.SaveChanges();
				// 行级排他锁结束
                return new
                {
    ...
    public partial class GetTrafficEmptyEvaluator:Evaluator
    {
        protected override object Evaluate(CommonRequest request)
        {
            return new Traffic();
    ...
    public partial class GetTrafficListEvaluator : Evaluator
    {
        protected override object Evaluate(CommonRequest request)
        {
            using (var ctx = new SPCall())
            {
                ...
                if (sppara != null)
                {
                    rows=ctx.SP_GET_ALL_LIST<Traffic>(sppara);
                    return new
                    {
                        success = true,
                        rows = rows,
                ...
                rows = ctx.SP_GET_ALL_LIST<Traffic>("", "", "", "", "", 0, 10, out total);
                return new
                {
                    success = true,
                    rows = rows,
