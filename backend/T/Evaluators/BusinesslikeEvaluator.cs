...
{
    public partial class DeleteBusinesslikeEvaluator : Evaluator
    {
    ...
    public partial class SaveBusinesslikeEvaluator : Evaluator
    {
        protected override object Evaluate(CommonRequest request)
        {
            if (Sessions.Count==0)
    ...
    public partial class GetBusinesslikeEmptyEvaluator:Evaluator
    {
        protected override object Evaluate(CommonRequest request)
        {
            return new Businesslike();
        }
    }

    public partial class GetBusinesslikeListEvaluator : Evaluator
    {
        protected override object Evaluate(CommonRequest request)
        {
            using (var ctx = new SPCall())
            {
                ...
                Businesslike[] rows = null;
                if (sppara != null)
                {
                    rows=ctx.SP_GET_ALL_LIST<Businesslike>(sppara);
                    return new
                    {
                        success = true,
                        rows = rows,
                ...
                rows = ctx.SP_GET_ALL_LIST<Businesslike>("", "", "", "", "", 0, 10, out total);
                return new
                {
                    success = true,
                    rows = rows,
