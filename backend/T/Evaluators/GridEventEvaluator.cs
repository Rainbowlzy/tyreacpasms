...
{
    public partial class DeleteGridEventEvaluator : Evaluator
    {
        protected override object Evaluate(CommonRequest request)
        {
            var data = JsonConvert.DeserializeObject<GridEvent>(request.data);
            if (data == null)
            ...
            {
                var one = ctx.GridEvent.Find(data.id);
				if(one==null){
					return new CommonOutputT<string>
					{
						success = false,
    ...
    public partial class SaveGridEventEvaluator : Evaluator
    {
        protected override object Evaluate(CommonRequest request)
        {
            if (Sessions.Count==0)
            ...
            {
				var s = request.data;
				var entity = JsonConvert.DeserializeObject<GridEvent>(s);
				// 行级排他锁开始
                var transactionId = Guid.NewGuid().ToString();
                var isnew = entity.id == null;
                if (!isnew)
                {
                    var one = ctx.GridEvent.FirstOrDefault(p=>p.id==entity.id);
                ...
                entity.UpdateOn = now;
                entity.IsDeleted = 0;
				ctx.GridEvent.AddOrUpdate(entity);
                ctx.SaveChanges();
				// 行级排他锁结束
                return new
                {
    ...
    public partial class GetGridEventEmptyEvaluator:Evaluator
    {
        protected override object Evaluate(CommonRequest request)
        {
    ...
    public partial class GetGridEventListEvaluator : Evaluator
    {
        protected override object Evaluate(CommonRequest request)
        {
            using (var ctx = new SPCall())
            {
                ...
                if (sppara != null)
                {
                    rows=ctx.SP_GET_ALL_LIST<GridEvent>(sppara);
                    return new
                    {
                        success = true,
                        rows = rows,
                ...
                rows = ctx.SP_GET_ALL_LIST<GridEvent>("", "", "", "", "", 0, 10, out total);
                return new
                {
                    success = true,
                    rows = rows,
