        ...
        protected override object Evaluate(CommonRequest request)
        {
            ...
            using (var ctx = new DefaultContext())
            {
				...
				entity.UIHeadPortrait = HttpUtility.UrlDecode(entity.UIHeadPortrait);
					// NVARCHAR(50) 所属部门
				entity.UISubordinateDepartment = HttpUtility.UrlDecode(entity.UISubordinateDepartment);
					// NVARCHAR(50) 电话
    ...
    public partial class GetUserInformationListEvaluator : Evaluator
    {
        ...
        protected override object Evaluate(CommonRequest request)
        {
            using (var ctx = new DefaultContext())
            {
				...
				// UISubordinateDepartment NVARCHAR(50) 所属部门 
                if(!string.IsNullOrEmpty(searchModel.UISubordinateDepartment)) query = query.Where(t=>t.UISubordinateDepartment.Contains(searchModel.UISubordinateDepartment));
                if(sort=="UISubordinateDepartment")
                {
					query = order=="asc"?query.OrderBy(t=>t.UISubordinateDepartment):query.OrderByDescending(t=>t.UISubordinateDepartment);
                    isordered = true;
                }
				...
				if(!string.IsNullOrEmpty(search)){
					query = query.Where(t=>t.id!=-1||t.UILoginName.Contains(search)||t.UINickname.Contains(search)||t.UIRealName.Contains(search)||t.UIHeadPortrait.Contains(search)||t.UISubordinateDepartment.Contains(search)||t.UIBooth.Contains(search)||t.UIPhoto.Contains(search)||t.UICustomerType.Contains(search)||t.UIUserLevel.Contains(search)||t.UICode.Contains(search));
				}
                if(sort=="ord")
                {
					query = order=="asc"?query.OrderBy(t=>t.ord):query.OrderByDescending(t=>t.ord);
            ...
            using (var ctx = new DefaultContext())
            {
                ...
                if (!isnew)
                {
                    ...
                    entity.VersionNo = one.VersionNo;
								...
								// NVARCHAR(50) 工号
				entity.SJobNumber = HttpUtility.UrlDecode(entity.SJobNumber);
					// NVARCHAR(50) 姓名
				entity.SName = HttpUtility.UrlDecode(entity.SName);
					// NVARCHAR(50) 公司
				entity.SAffiliate = HttpUtility.UrlDecode(entity.SAffiliate);
					// NVARCHAR(50) 部门
				entity.SDepartment = HttpUtility.UrlDecode(entity.SDepartment);
					// NVARCHAR(50) 职位
				entity.SPost = HttpUtility.UrlDecode(entity.SPost);
					// NVARCHAR(50) 联系方式
				entity.SCommonModeOfContact = HttpUtility.UrlDecode(entity.SCommonModeOfContact);
					// NVARCHAR(50) 备用联系方式
				entity.SAlternateContactMode = HttpUtility.UrlDecode(entity.SAlternateContactMode);
					// NVARCHAR(50) 密码
				entity.SCode = HttpUtility.UrlDecode(entity.SCode);
    ...
    public partial class GetStaffnameListEvaluator : Evaluator
    {
        ...
        protected override object Evaluate(CommonRequest request)
        {
            using (var ctx = new DefaultContext())
            {
                ...
                var query = ctx.Staffname.Where(t=>t.IsDeleted==0 && t.DataLevel.StartsWith(datalevel));
                var @params = request.context.Request.Params;
                searchModel.PageSize = (@params["limit"] ?? searchModel.PageSize.ToString()).ToInt();
				searchModel.PageSize = searchModel.PageSize==0?4000:searchModel.PageSize;
                searchModel.PageIndex = (@params["offset"]).ToInt()/ searchModel.PageSize;
                ...
                var sort = searchModel.Sort ?? @params["sort"];
				var order = @params["order"];
				// SJobNumber NVARCHAR(50) 工号 
                if(!string.IsNullOrEmpty(searchModel.SJobNumber)) query = query.Where(t=>t.SJobNumber.Contains(searchModel.SJobNumber));
                if(sort=="SJobNumber")
                {
				...
				// SAffiliate NVARCHAR(50) 公司 
                if(!string.IsNullOrEmpty(searchModel.SAffiliate)) query = query.Where(t=>t.SAffiliate.Contains(searchModel.SAffiliate));
                if(sort=="SAffiliate")
                {
					query = order=="asc"?query.OrderBy(t=>t.SAffiliate):query.OrderByDescending(t=>t.SAffiliate);
                    isordered = true;
                }
				// SDepartment NVARCHAR(50) 部门 
                if(!string.IsNullOrEmpty(searchModel.SDepartment)) query = query.Where(t=>t.SDepartment.Contains(searchModel.SDepartment));
                if(sort=="SDepartment")
                {
					query = order=="asc"?query.OrderBy(t=>t.SDepartment):query.OrderByDescending(t=>t.SDepartment);
                    isordered = true;
                }
				// SPost NVARCHAR(50) 职位 
                if(!string.IsNullOrEmpty(searchModel.SPost)) query = query.Where(t=>t.SPost.Contains(searchModel.SPost));
                if(sort=="SPost")
                {
					query = order=="asc"?query.OrderBy(t=>t.SPost):query.OrderByDescending(t=>t.SPost);
                    isordered = true;
                }
				...
				// SAlternateContactMode NVARCHAR(50) 备用联系方式 
                if(!string.IsNullOrEmpty(searchModel.SAlternateContactMode)) query = query.Where(t=>t.SAlternateContactMode.Contains(searchModel.SAlternateContactMode));
                if(sort=="SAlternateContactMode")
                {
					query = order=="asc"?query.OrderBy(t=>t.SAlternateContactMode):query.OrderByDescending(t=>t.SAlternateContactMode);
                    isordered = true;
                }
				// STimeOfEntry DATETIME 入职时间 
                if(searchModel.FromSTimeOfEntry!=null) query = query.Where(t=>t.STimeOfEntry>=searchModel.FromSTimeOfEntry);
                if(searchModel.ToSTimeOfEntry!=null) query = query.Where(t=>t.STimeOfEntry<=searchModel.ToSTimeOfEntry);
                if(sort=="STimeOfEntry")
                {
					query = order=="asc"?query.OrderBy(t=>t.STimeOfEntry):query.OrderByDescending(t=>t.STimeOfEntry);
                    isordered = true;
                }
				// SDepartureTime DATETIME 离职时间 
                if(searchModel.FromSDepartureTime!=null) query = query.Where(t=>t.SDepartureTime>=searchModel.FromSDepartureTime);
                if(searchModel.ToSDepartureTime!=null) query = query.Where(t=>t.SDepartureTime<=searchModel.ToSDepartureTime);
                if(sort=="SDepartureTime")
                {
					query = order=="asc"?query.OrderBy(t=>t.SDepartureTime):query.OrderByDescending(t=>t.SDepartureTime);
                    isordered = true;
                }
				// SCode NVARCHAR(50) 密码 
                if(!string.IsNullOrEmpty(searchModel.SCode)) query = query.Where(t=>t.SCode.Contains(searchModel.SCode));
                if(sort=="SCode")
                {
					query = order=="asc"?query.OrderBy(t=>t.SCode):query.OrderByDescending(t=>t.SCode);
                    isordered = true;
                }
				if(!string.IsNullOrEmpty(search)){
					query = query.Where(t=>t.id!=-1||t.SJobNumber.Contains(search)||t.SName.Contains(search)||t.SAffiliate.Contains(search)||t.SDepartment.Contains(search)||t.SPost.Contains(search)||t.SCommonModeOfContact.Contains(search)||t.SAlternateContactMode.Contains(search)||t.SCode.Contains(search));
				}
                if(sort=="ord")
                {
					query = order=="asc"?query.OrderBy(t=>t.ord):query.OrderByDescending(t=>t.ord);
        ...
        protected override object Evaluate(CommonRequest request)
        {
            ...
            using (var ctx = new DefaultContext())
            {
				...
				entity.CName = HttpUtility.UrlDecode(entity.CName);
					// NVARCHAR(50) 名
				entity.CCard = HttpUtility.UrlDecode(entity.CCard);
					// NVARCHAR(50) 性别
    ...
    public partial class GetCustomertypeListEvaluator : Evaluator
    {
        ...
        protected override object Evaluate(CommonRequest request)
        {
            using (var ctx = new DefaultContext())
            {
				...
				// CCard NVARCHAR(50) 名 
                if(!string.IsNullOrEmpty(searchModel.CCard)) query = query.Where(t=>t.CCard.Contains(searchModel.CCard));
                if(sort=="CCard")
                {
					query = order=="asc"?query.OrderBy(t=>t.CCard):query.OrderByDescending(t=>t.CCard);
                    isordered = true;
                }
				...
				if(!string.IsNullOrEmpty(search)){
					query = query.Where(t=>t.id!=-1||t.CCustomerNumber.Contains(search)||t.CName.Contains(search)||t.CCard.Contains(search)||t.CChairperson.Contains(search)||t.CCall.Contains(search)||t.CCommonModeOfContact.Contains(search)||t.CAddress.Contains(search));
				}
                if(sort=="ord")
                {
					query = order=="asc"?query.OrderBy(t=>t.ord):query.OrderByDescending(t=>t.ord);
