...
{
    public partial class DeleteClientEvaluator : Evaluator
    {
        protected override object Evaluate(CommonRequest request)
        {
            var data = JsonConvert.DeserializeObject<Client>(request.data);
            if (data == null)
            ...
            {
                var one = ctx.Client.Find(data.id);
				if(one==null){
					return new CommonOutputT<string>
					{
						success = false,
    ...
    public partial class SaveClientEvaluator : Evaluator
    {
        protected override object Evaluate(CommonRequest request)
        {
            if (Sessions.Count==0)
            ...
            {
				var s = request.data;
				var entity = JsonConvert.DeserializeObject<Client>(s);
				// 行级排他锁开始
                var transactionId = Guid.NewGuid().ToString();
                var isnew = entity.id == null;
                if (!isnew)
                {
                    var one = ctx.Client.FirstOrDefault(p=>p.id==entity.id);
    ...
    public partial class GetClientEmptyEvaluator:Evaluator
    {
        protected override object Evaluate(CommonRequest request)
        {
            return new Client();
    ...
    public partial class GetClientListEvaluator : Evaluator
    {
        protected override object Evaluate(CommonRequest request)
        {
            using (var ctx = new SPCall())
            {
                ...
                if (sppara != null)
                {
                    rows=ctx.SP_GET_ALL_LIST<Client>(sppara);
                    return new
                    {
                        success = true,
                        rows = rows,
                ...
                rows = ctx.SP_GET_ALL_LIST<Client>("", "", "", "", "", 0, 10, out total);
                return new
                {
                    success = true,
                    rows = rows,
