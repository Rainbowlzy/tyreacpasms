    ...
    $(document).ready(function() {
        ...
        var method = {
            ...
            save() {
                ...
                $.call("SaveLogonRecord", JSON.parse(JSON.stringify(datum)), function(data) {
			...
			back(){
				window.location.href = site_gen+"LogonRecord.html?table=LogonRecord"
			},
			...
			if (!request || request.id === undefined) {
				$.call("GetLogonRecordEmpty", {}, function(data) {
					data = data||{};
					data.table = 'LogonRecord'
					vm = new Vue({
						el: "#LogonRecord",
						data: data||{},
						computed:{
				...
				$.call("GetLogonRecordList", {SearchKey: request.id}, function(data) {
					if(!data){
						alert(data);
						return;
					}
